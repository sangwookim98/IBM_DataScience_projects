...
site = 'https://www.macrotrends.net/stocks/charts/TSLA/tesla/revenue?utm_medium=Exinfluencer&utm_source=Exinfluencer&utm_content=000026UJ&utm_term=10006555&utm_id=NA-SkillsNetwork-Channel-SkillsNetworkCoursesIBMDeveloperSkillsNetworkPY0220ENSkillsNetwork23455606-2021-01-01'
html_data = requests.get(site).text
...
# locating the specific table
revenue_tables = beautiful_soup.find_all('table')

# getting the index number for that one table among all the tables in the page
for index, table in enumerate(revenue_tables):
    if ("Tesla Quarterly Revenue" in str(table)):
        quarterly_table = index

# setting up the header names
tesla_revenue = pd.DataFrame(columns=["Date", "Revenue"])

# revenue_tables[quarterly_table]
for row in revenue_tables[quarterly_table].tbody.find_all("tr"):
    col = row.find_all("td")
    Date = col[0].text
    Revenue = col[1].text
...
revenue_tables = soup.find_all('table')
